...
class Insertion_auto_contr extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('insertion_auto_model');
// Alternately you can set preferences by calling the initialize function. Useful if you auto-load the class:
    }
    /////////////////
    function index()
    {
        $this->load->view('insertion_auto');
    }
    //////////////
                ...
                //mot de pass correct, present
                    ...
                    //////////////////////////////////////
                       $command= substr($chemin,-7,7);
                        $ver= $this->insertion_auto_model->get_verrification($command);
                        if($ver==1)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                $destination="C:/images/traitement";//asiana ny image dy destination(lien)
                                ...
                                $this->insertion_auto_model->get_insertionPre($command,$Total,$nombre,$lien,$matricule,$traitement,$export,$idcommande,$fic);
                            $preparation = $this->insertion_auto_model->get_selectPre($command);
                           ...
                           $Seq=10000;
                                for($Isa=0;$Isa<$Total;$Isa++)
                                {
                                 ...
                                 if($nombre== 1)
                                 {
                                     ...
                                     $RepertoireACreer="$destination\\$TroisLetre\\$command\\$command$Sequence";
                                     $this->insertion_auto_model->get_insertionFICHIER($SousRepert,$command,$rep,$RepertoireACreer,$id_preparation,$traitement,$export,$idcommande,$fic,$dossier);
                                     $this->insertion_auto_model->get_insertionLISTAGE($rep,$command,$SousRepert,$id_preparation,$RepertoireACreer,$export,$idcommande,$fic);
                                 }
                                 else
                                 {
                         if($Manomboka==1)
